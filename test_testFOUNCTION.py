...
def update_lines(screen, near_by_list, obj, position, init, zoom):
    ...
    for j in range(9):
        ...
        vertical = math.cos(radians) * (100 * zoom)
        horizontal = math.sin(radians) * (100 * zoom)
        ...
        for k in range(len(near_by_list)):
            ...
            if coordinates:
                ...
                if type(near_by_list[k]) == Predator:
                    obj.input[j * 2] = abs(coordinates[0][0] - new_line[0][0]) + abs(coordinates[0][1] - new_line[0][1])
...
def set_preys_predators(init):
    ...
    for e in range(int(init.data[0])):
        prey = Class.Prey(float(init.data[1]), float(init.data[2]))
    ...
    for e in range(int(init.data[3])):
        predator = Class.Predator(float(init.data[4]), float(init.data[5]))
...
def spawn_more_obj(init, preys, predators):
    ...
    for e in range(50):
        ...
        predator.angle = random.randrange(0, 360)
        init.data[3] = str(int(init.data[0]) + 1)
        predators.append(predator)
    return preys, predators
    ...
    for i in range(len(init.predators)):
        ...
        if init.selected[1] == predator:
            for l in range(len(list_rect)):
                pygame.draw.rect(init.screen, (255, 255, 255), (list_rect[l].left - init.position[0], list_rect[l].top - init.position[1], 10, 10), 2)
        if index != None:
            for element in index:
                obj = list[element]
                if type(obj) == Prey:
